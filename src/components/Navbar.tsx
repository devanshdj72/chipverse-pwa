...
export default function Navbar() {
  ...
  // When sidebar opens/closes, update CSS variable on html element
  // This lets main content shift without needing shared state
  useEffect(() => {
    const root = document.documentElement;
    if (isAuthenticated && open) {
      root.style.setProperty('--sidebar-offset', '240px');
    } else {
  ...
  useEffect(() => { if (!isAuthenticated) setOpen(false); }, [isAuthenticated]);
  ...
  // How much to shift content when sidebar open
  const shift = isAuthenticated && open ? SIDEBAR_W : 0;

  return (
    <>
      <style>{`
        @keyframes slideInToast { from{opacity:0;transform:translateX(60px)} to{opacity:1;transform:translateX(0)} }
        .cv-sidebar  { transition: transform 0.26s cubic-bezier(0.4,0,0.2,1); }
        .cv-topbar   { transition: left 0.26s cubic-bezier(0.4,0,0.2,1), right 0.26s cubic-bezier(0.4,0,0.2,1); }
        .cv-main     { transition: margin-left 0.26s cubic-bezier(0.4,0,0.2,1); }
      `}</style>
      ...
      {isAuthenticated && (
        <aside
          className="cv-sidebar fixed top-0 left-0 h-full z-50 flex flex-col bg-[#090912] border-r border-white/8 overflow-hidden"
          style={{
            width: SIDEBAR_W,
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-4 border-b border-white/8 shrink-0">
            <Link href="/" className="flex items-center gap-2.5 hover:opacity-80 transition-opacity">
              <Microchip className="w-5 h-5 text-blue-500 shrink-0" />
              <span className="font-bold text-base font-['Orbitron'] text-white whitespace-nowrap">ChipVerse</span>
            </Link>
            <button onClick={() => setOpen(false)}
              className="text-gray-500 hover:text-white transition-colors p-1 rounded-lg hover:bg-white/8">
              <X size={16} />
            </button>
          </div>
          ...
          <nav className="flex-1 py-3 overflow-y-auto">
            {NAV_ITEMS.map(({ href, label, icon: Icon }) => {
              ...
              return (
                <Link key={href} href={href}
                  ...
                  <Icon size={17} className={cn("shrink-0 transition-colors", active ? "text-blue-400" : "text-gray-500 group-hover:text-gray-300")} />
                  <span>{label}</span>
            ...
            <button onClick={handleOdysseyReplay}
              className="w-full flex items-center gap-3 px-5 py-2.5 text-sm font-medium text-purple-400 hover:bg-purple-500/8 hover:text-purple-300 transition-all whitespace-nowrap">
          ...
          <div className="border-t border-white/8 p-3 shrink-0">
            <div className="flex items-center gap-2.5 px-2 py-2 rounded-xl bg-white/4 mb-2">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-gray-500 text-[10px]">⚡ {profile?.xp ?? 0} · 🔥 {profile?.streak ?? 0}</p>
              </div>
            </div>
      ...
      {/* ── Top bar ── */}
      <header
        className="cv-topbar fixed top-0 z-40 h-14 bg-black/70 backdrop-blur-md border-b border-white/8 flex items-center px-4 gap-3"
        style={{ left: shift, right: 0 }}
      >
        {/* Hamburger — only when logged in */}
        {isAuthenticated && (
          <button
            onClick={() => setOpen(v => !v)}
            className="flex items-center justify-center w-8 h-8 rounded-lg text-gray-400 hover:text-white hover:bg-white/8 transition-all shrink-0"
        ...
        <Link href="/" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
          ...
          <span className="font-bold text-sm font-['Orbitron'] text-white hidden sm:block">ChipVerse</span>
        </Link>
        ...
        {isAuthenticated ? (
          <div className="flex items-center gap-2">
            ...
            <Link href="/messages"
              ...
              {unreadMessages > 0 && (
                <span className="absolute top-1 right-1 w-3.5 h-3.5 bg-green-500 rounded-full text-[8px] font-bold text-white flex items-center justify-center">
                    ...
                    {notifications.length === 0
                      ...
                      : notifications.map(n => (
                        <div key={n.id}
                          ...
                          <span className="text-lg shrink-0">{({ friend_request: "👥", friend_accepted: "✅", battle_challenge: "⚔️", battle_result: "🏆" } as any)[n.type] ?? "🔔"}</span>
            ...
            {/* Avatar */}
            <Link href="/profile">
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-600 to-purple-600 flex items-center justify-center text-sm font-bold text-white cursor-pointer hover:ring-2 hover:ring-blue-500/50 transition-all">
                {user?.name?.charAt(0)?.toUpperCase() ?? "U"}
              </div>
            </Link>
          </div>
