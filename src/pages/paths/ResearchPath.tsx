...
import { ROADMAPS, RESEARCH_SUB_LEVELS } from "@/lib/data";
import { RANKS } from "@/lib/ranks";
import CircuitBackground from "@/components/CircuitBackground";
import ParticleCanvas from "@/components/ParticleCanvas";
import SidebarWidget from "@/components/SidebarWidget";
import ProgressBar from "@/components/ProgressBar";
import ResearchSubLevelPanel from "@/components/ResearchSubLevelPanel";
import { cn } from "@/lib/utils";

const DOMAIN_ID = "research";
const STORAGE_KEY = "research_sublevel_progress";
...
function loadProgress(): SubLevelProgress {
  try {
...
function saveProgress(p: SubLevelProgress) {
...
export default function ResearchPath() {
  ...
  const { isSubscribed, isLoading: subLoading, price } = useSubscription("research");
  ...
  return (
    <div className="min-h-screen pt-16 relative bg-black overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
            ...
            <p className="text-gray-400 text-lg max-w-xl">
              Deep research roadmap — each level unlocks 4 sub-levels with real content, quizzes, and XP.
            </p>
            <div style={{ display: "flex", gap: "12px", marginTop: "14px", flexWrap: "wrap" }}>
              <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: "999px", padding: "4px 14px", fontSize: "11px", fontFamily: "'DM Mono', monospace", color: theme.primary, fontWeight: 700 }}>{levels.length} Levels × 4 Sub-levels</div>
              <div style={{ background: "rgba(234,179,8,0.1)", border: "1px solid rgba(234,179,8,0.25)", borderRadius: "999px", padding: "4px 14px", fontSize: "11px", fontFamily: "'DM Mono', monospace", color: "#eab308", fontWeight: 700 }}>+{progress.totalXp} XP Earned</div>
              <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "999px", padding: "4px 14px", fontSize: "11px", fontFamily: "'DM Mono', monospace", color: "#888" }}>{progress.completedSubLevels.length}/{levels.length * 4} Sub-levels</div>
            </div>
          </div>
          <div className="flex gap-4 flex-wrap items-start">
            ...
            <div className="bg-black/50 backdrop-blur-md border border-white/10 rounded-2xl p-5 min-w-[150px]">
              <div className="text-gray-400 text-sm mb-2 uppercase tracking-wider">Rank</div>
            ...
            <a href={`${import.meta.env.BASE_URL}report/research`} className="flex items-center gap-2 px-5 py-3 rounded-2xl font-semibold text-sm transition-all hover:opacity-90 active:scale-95"
      ...
      <div style={{ background: `${theme.card}`, borderBottom: `1px solid ${theme.border}`, padding: "10px 24px", textAlign: "center" }}>
        <span style={{ color: theme.primary, fontSize: "11px", fontFamily: "'DM Mono', monospace", fontWeight: 600 }}>
          💡 Click any active level to unlock its 4 sub-levels — Background → Techniques → Research Angles → Research Paper
        </span>
      </div>
        ...
        <div className="flex-1 relative">
          ...
          <div className="relative">
            {levels.map((level, idx) => {
              ...
              const levelSubData = RESEARCH_SUB_LEVELS.find((d) => d.levelId === level.id);
              const completedSubCount = levelSubData ? levelSubData.subLevels.filter((sl) => progress.completedSubLevels.includes(sl.id)).length : 0;
                ...
                <div key={level.id} className="relative">
                  <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.04 }}
                    ...
                    <div onClick={() => {
                      ...
                      {status === "completed" && <Check style={{ width: "20px", height: "20px", color: theme.primary }} />}
                      {status === "active" && !isActive && <Play style={{ width: "18px", height: "18px", marginLeft: "2px", color: theme.primary }} />}
                      ...
                      {status !== "locked" && levelSubData && (
                        <div style={{ marginTop: "10px" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                            <span style={{ color: "#555", fontSize: "9.5px", fontFamily: "'DM Mono', monospace" }}>Sub-levels</span>
                            <span style={{ color: theme.primary, fontSize: "9.5px", fontFamily: "'DM Mono', monospace", fontWeight: 700 }}>{completedSubCount}/4</span>
                          </div>
                          <div style={{ display: "flex", gap: "3px" }}>
                            {["bg", "tech", "angles", "paper"].map((_, i) => {
                              const sl = levelSubData.subLevels[i];
                              const done = sl && progress.completedSubLevels.includes(sl.id);
                              return <div key={i} style={{ flex: 1, height: "4px", borderRadius: "999px", background: done ? theme.gradient : "rgba(255,255,255,0.08)", transition: "all 0.4s", boxShadow: done ? `0 0 4px ${theme.glow}` : "none" }} />;
                            })}
                          </div>
                        </div>
                      )}
                      {isActive && <div style={{ marginTop: "10px", display: "flex", alignItems: "center", gap: "6px", color: theme.primary, fontSize: "10px", fontFamily: "'DM Mono', monospace", fontWeight: 700 }}><Star style={{ width: "10px", height: "10px" }} /> Sub-levels expanded →</div>}
                    </motion.div>
                  </motion.div>
                </div>
              );
        ...
        <div className="w-full lg:w-80 flex flex-col gap-6 lg:sticky lg:top-24 h-fit">
          ...
          {progress.completedLevels.length > 0 && (
            <SidebarWidget title="Badges Earned">
              <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                {RESEARCH_SUB_LEVELS.filter((d) => progress.completedLevels.includes(d.levelId)).map((d) => (
                  <div key={d.levelId} style={{ display: "flex", alignItems: "center", gap: "8px", padding: "8px 10px", borderRadius: "10px", background: theme.card, border: `1px solid ${theme.border}` }}>
                    <span style={{ fontSize: "16px" }}>🏅</span>
                    <span style={{ color: theme.primary, fontSize: "11.5px", fontFamily: "'DM Mono', monospace", fontWeight: 700 }}>{d.badge}</span>
                  </div>
                ))}
              </div>
            </SidebarWidget>
          ...
          <SidebarWidget title="Skill Report">
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              ...
              <a href={`${import.meta.env.BASE_URL}report/research`} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", padding: "9px 16px", borderRadius: "10px", background: theme.gradient, color: "#000", fontFamily: "'DM Mono', monospace", fontSize: "11px", fontWeight: 700, textDecoration: "none", boxShadow: `0 0 14px ${theme.glow}55` }}>
      ...
      <AnimatePresence>
        {activeLevelIdx !== null && activeLevelData && activeLevel && (
          <ResearchSubLevelPanel levelData={activeLevelData} levelTitle={activeLevel.title} levelIndex={activeLevelIdx} theme={theme} domain="research"
