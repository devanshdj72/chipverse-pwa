...
const DOMAIN_ID = "verification";
const STORAGE_KEY = "verification_sublevel_progress";
...
export default function VerificationPath() {
  ...
  const [verSubLevels, setVerSubLevels] = useState<any[]>([]);
  ...
  const isPrevLevelFullyComplete = (idx: number): boolean => {
    ...
    const prevSubData = verSubLevels.find((d) => d.levelId === prevLevel.id);
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
            ...
            <p className="text-gray-400 text-lg max-w-xl">
              Master chip verification — SystemVerilog, UVM, assertions, coverage, and industry protocols.
            </p>
        ...
        <div className="flex-1 relative">
          ...
          <div className="relative">
            {levels.map((level, idx) => {
              ...
              const levelSubData = verSubLevels.find((d) => d.levelId === level.id);
          ...
          {progress.completedLevels.length > 0 && (
            <SidebarWidget title="Badges Earned">
              <div style={{ display: "flex", flexDirection: "column", gap: "7px" }}>
                {verSubLevels.filter((d) => progress.completedLevels.includes(d.levelId)).map((d) => (
