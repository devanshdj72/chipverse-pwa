...
const DOMAIN_ID = "rtl";
const STORAGE_KEY = "rtl_sublevel_progress";
...
export default function RTLPath() {
  ...
  const { isSubscribed, isLoading: subLoading, price } = useSubscription("rtl");
    ...
    <div className="min-h-screen pt-16 relative bg-black overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
            ...
            <p className="text-gray-400 text-lg max-w-xl">
              Master RTL design — each level has 5 sub-levels with concept, syntax, walkthrough, lab, and quiz.
            </p>
            <div style={{ display: "flex", gap: "10px", marginTop: "12px", flexWrap: "wrap" }}>
              ...
              <div style={{ background: "rgba(56,189,248,0.1)", border: "1px solid rgba(56,189,248,0.2)", borderRadius: "999px", padding: "3px 12px", fontSize: "10.5px", fontFamily: "'DM Mono', monospace", color: "#38bdf8", fontWeight: 700 }}>
                +{progress.totalXp} XP Earned
              </div>
              <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "999px", padding: "3px 12px", fontSize: "10.5px", fontFamily: "'DM Mono', monospace", color: "#888" }}>
                {progress.completedSubLevels.length}/{levels.length * 5} Sub-levels
              </div>
            </div>
          </div>
      ...
      <div style={{ background: theme.card, borderBottom: `1px solid ${theme.border}`, padding: "9px 24px", textAlign: "center" }}>
        <span style={{ color: theme.primary, fontSize: "10.5px", fontFamily: "'DM Mono', monospace", fontWeight: 600 }}>
          💡 Click any active level — Concept → Syntax → Walkthrough → Lab (Monaco Editor) → Quiz
        </span>
      </div>
          ...
          {progress.completedLevels.length > 0 && (
            <SidebarWidget title="Badges Earned">
              <div style={{ display: "flex", flexDirection: "column", gap: "7px" }}>
                {RTL_SUB_LEVELS.filter((d) => progress.completedLevels.includes(d.levelId)).map((d) => (
