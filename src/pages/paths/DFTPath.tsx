...
const DOMAIN_ID = "dft";
const STORAGE_KEY = "dft_sublevel_progress";
...
function loadProgress(): DFTProgress {
  try {
...
function saveProgress(p: DFTProgress) {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify(p)); } catch {}
}

export default function DFTPath() {

  const [, setLocation] = useLocation();
  const { isSubscribed, isLoading: subLoading, price } = useSubscription("dft");
  ...
  const [progress, setProgress] = useState<DFTProgress>(loadProgress);
  const [dftSubLevels, setDftSubLevels] = useState<any[]>([]);
  ...
  const isPrevLevelFullyComplete = (idx: number): boolean => {
    ...
    const prevSubData = dftSubLevels.find(d => d.levelId === prevLevel.id);
    ...
    <div className="min-h-screen pt-16 relative bg-black overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
            ...
            <p className="text-gray-400 text-lg max-w-xl">
              Master chip testability — scan chains, ATPG, MBIST, LBIST, JTAG, compression, and at-speed testing.
            </p>
            <div style={{ display: "flex", gap: "10px", marginTop: "12px", flexWrap: "wrap" }}>
              ...
              <div style={{ background: "rgba(168,85,247,0.1)", border: "1px solid rgba(168,85,247,0.2)", borderRadius: "999px", padding: "3px 12px", fontSize: "10.5px", fontFamily: "'DM Mono',monospace", color: "#a855f7", fontWeight: 700 }}>+{progress.totalXp} XP Earned</div>
              <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "999px", padding: "3px 12px", fontSize: "10.5px", fontFamily: "'DM Mono',monospace", color: "#888" }}>{progress.completedSubLevels.length}/{levels.length * 5} Sub-levels</div>
            </div>
          </div>
          <div className="flex gap-4 flex-wrap items-start">
            ...
            <div className="bg-black/50 backdrop-blur-md border border-white/10 rounded-2xl p-5 min-w-[150px]">
              ...
              <div className="text-xl font-bold mt-2" style={{ color: theme.primary }}>
                {RANKS[Math.min(Math.floor((completedIds.length + progress.completedLevels.length) / 2), RANKS.length - 1)]}
              </div>
            </div>
            <a href={`${import.meta.env.BASE_URL}report/dft`} className="flex items-center gap-2 px-5 py-3 rounded-2xl font-semibold text-sm transition-all hover:opacity-90 active:scale-95"
      ...
      <div style={{ background: theme.card, borderBottom: `1px solid ${theme.border}`, padding: "9px 24px", textAlign: "center" }}>
        <span style={{ color: theme.primary, fontSize: "10.5px", fontFamily: "'DM Mono',monospace", fontWeight: 600 }}>
          💡 Click any active level — Concept → Syntax → Walkthrough → Lab (SystemVerilog) → Quiz
        </span>
      </div>
        ...
        <div className="flex-1 relative">
          ...
          <div className="relative">
            {levels.map((level, idx) => {
              ...
              const levelSubData = dftSubLevels.find(d => d.levelId === level.id);
        ...
        <div className="w-full lg:w-80 flex flex-col gap-6 lg:sticky lg:top-24 h-fit">
          ...
          {progress.completedLevels.length > 0 && (
            <SidebarWidget title="Badges Earned">
              <div style={{ display: "flex", flexDirection: "column", gap: "7px" }}>
                {dftSubLevels.filter(d => progress.completedLevels.includes(d.levelId)).map(d => (
          ...
          <SidebarWidget title="Skill Report">
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              ...
              <a href={`${import.meta.env.BASE_URL}report/dft`} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", padding: "9px 16px", borderRadius: "10px", background: theme.gradient, color: "#000", fontFamily: "'DM Mono', monospace", fontSize: "11px", fontWeight: 700, textDecoration: "none", boxShadow: `0 0 14px ${theme.glow}55` }}>
      ...
      <AnimatePresence>
        {activeLevelIdx !== null && activeLevelData && activeLevel && (
          <RTLSubLevelPanel levelData={activeLevelData} levelTitle={activeLevel.title} levelIndex={activeLevelIdx} theme={theme} domain="dft"
