...
const DOMAIN_ID = "embedded";
const STORAGE_KEY = "embedded_sublevel_progress";
...
function loadProgress(): EmbProgress {
  try {
...
function saveProgress(p: EmbProgress) {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify(p)); } catch {}
}

export default function EmbeddedPath() {

  const [, setLocation] = useLocation();
  const { isSubscribed, isLoading: subLoading, price } = useSubscription("embedded");
  ...
  const [progress, setProgress] = useState<EmbProgress>(loadProgress);
  const [embSubLevels, setEmbSubLevels] = useState<any[]>([]);
  const [activeLevelIdx, setActiveLevelIdx] = useState<number | null>(null);

  useEffect(() => {
    fetch(`${import.meta.env.BASE_URL}data/embedded-sublevels.json`).then(r => r.json()).then(setEmbSubLevels).catch(console.error);
  }, []);
  ...
  const isPrevLevelFullyComplete = (idx: number): boolean => {
    ...
    const prevSubData = embSubLevels.find(d => d.levelId === prevLevel.id);
  ...
  const activeLevelData = activeLevelIdx !== null ? embSubLevels.find(d => d.levelId === levels[activeLevelIdx].id) : null;
    ...
    <div className="min-h-screen pt-16 relative bg-black overflow-x-hidden">
      ...
      <div style={{ background: theme.card, borderBottom: `1px solid ${theme.border}`, padding: "9px 24px", textAlign: "center" }}>
        <span style={{ color: theme.primary, fontSize: "10.5px", fontFamily: "'DM Mono',monospace", fontWeight: 600 }}>
          💡 Labs in C — Concept → Syntax → Walkthrough → Lab → Quiz
        </span>
      </div>
        ...
        <div className="flex-1 relative">
          ...
          <div className="relative">
            {levels.map((level, idx) => {
              ...
              const levelSubData = embSubLevels.find(d => d.levelId === level.id);
          ...
          {progress.completedLevels.length > 0 && (
            <SidebarWidget title="Badges Earned">
              <div style={{ display: "flex", flexDirection: "column", gap: "7px" }}>
                {embSubLevels.filter(d => progress.completedLevels.includes(d.levelId)).map(d => (
