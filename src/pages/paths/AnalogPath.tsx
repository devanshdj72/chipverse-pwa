...
const DOMAIN_ID = "analog";
const STORAGE_KEY = "analog_sublevel_progress";
...
function loadProgress(): AnalogProgress {
  try {
...
function saveProgress(p: AnalogProgress) {
...
export default function AnalogPath() {
  ...
  const [progress, setProgress] = useState<AnalogProgress>(loadProgress);
  const [analogSubLevels, setAnalogSubLevels] = useState<any[]>([]);
  ...
  const isPrevLevelFullyComplete = (idx: number): boolean => {
    ...
    const prevSubData = analogSubLevels.find(d => d.levelId === prevLevel.id);
  ...
  const activeLevelData = activeLevelIdx !== null ? analogSubLevels.find(d => d.levelId === levels[activeLevelIdx].id) : null;
    ...
    <div className="min-h-screen pt-16 relative bg-black overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
            ...
            <p className="text-gray-400 text-lg max-w-xl">
              Master analog IC design — MOSFET analysis, op-amps, current mirrors, PLLs, ADCs, noise, and layout.
            </p>
            <div style={{ display: "flex", gap: "10px", marginTop: "12px", flexWrap: "wrap" }}>
              ...
              <div style={{ background: "rgba(245,158,11,0.1)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: "999px", padding: "3px 12px", fontSize: "10.5px", fontFamily: "'DM Mono',monospace", color: "#f59e0b", fontWeight: 700 }}>Circuit Analysis Labs</div>
          ...
          <div className="flex gap-4 flex-wrap items-start">
            ...
            <a href={`${import.meta.env.BASE_URL}report/analog`} className="flex items-center gap-2 px-5 py-3 rounded-2xl font-semibold text-sm transition-all hover:opacity-90 active:scale-95"
      ...
      <div style={{ background: theme.card, borderBottom: `1px solid ${theme.border}`, padding: "9px 24px", textAlign: "center" }}>
        <span style={{ color: theme.primary, fontSize: "10.5px", fontFamily: "'DM Mono',monospace", fontWeight: 600 }}>
          💡 Labs are circuit analysis calculations — Concept → Key Equations → Walkthrough → Lab → Quiz
        </span>
      </div>
        ...
        <div className="flex-1 relative">
          ...
          <div className="relative">
            {levels.map((level, idx) => {
              ...
              const levelSubData = analogSubLevels.find(d => d.levelId === level.id);
        ...
        <div className="w-full lg:w-80 flex flex-col gap-6 lg:sticky lg:top-24 h-fit">
          ...
          {progress.completedLevels.length > 0 && (
            <SidebarWidget title="Badges Earned">
              <div style={{ display: "flex", flexDirection: "column", gap: "7px" }}>
                {analogSubLevels.filter(d => progress.completedLevels.includes(d.levelId)).map(d => (
          ...
          <SidebarWidget title="Skill Report">
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              ...
              <a href={`${import.meta.env.BASE_URL}report/analog`} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", padding: "9px 16px", borderRadius: "10px", background: theme.gradient, color: "#000", fontFamily: "'DM Mono', monospace", fontSize: "11px", fontWeight: 700, textDecoration: "none", boxShadow: `0 0 14px ${theme.glow}55` }}>
      ...
      <AnimatePresence>
        {activeLevelIdx !== null && activeLevelData && activeLevel && (
          <RTLSubLevelPanel levelData={activeLevelData} levelTitle={activeLevel.title} levelIndex={activeLevelIdx} theme={theme} domain="analog"
